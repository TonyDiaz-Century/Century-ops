  ...
  roof: {
    ...
    buildTasks(answers) {
      ...
      const t = [
        ...
        { text: 'Order paint to match', critical: false },
      ];
      ...
      t.push(
        ...
        { text: 'Install Batts', critical: true },
...
async function tsm_confirmSave() {
  ...
  if (btn.disabled) return; // prevent double-save
  ...
  try {
    ...
    // Small delay to ensure deletes are committed before writing
...
async function clearExistingTasks(jobName, token, baseId, tableId) {
  // Find existing tasks for this job using encoded filter
  const formula = encodeURIComponent(`{Job}="${jobName}"`);
  const url = `https://api.airtable.com/v0/${baseId}/${tableId}?filterByFormula=${formula}&fields[]=Task`;
