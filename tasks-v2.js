...
async function tsm_confirmSave() {
  ...
  const AIRTABLE_TOKEN = 'patZMOjIXsZ40w3UT';
  const BASE_ID = 'appn1m7nkB7AqfMsN';
  const TASKS_TABLE = 'tblaB0sGkfDujQr43';

  try {
    // Delete existing tasks for this job first
    await clearExistingTasks(_tsm_jobId, AIRTABLE_TOKEN, BASE_ID, TASKS_TABLE);
...
async function clearExistingTasks(jobId, token, baseId, tableId) {
  ...
  const url = `https://api.airtable.com/v0/${baseId}/${tableId}?filterByFormula=FIND("${jobId}",ARRAYJOIN({Job}))&fields[]=Task+Name`;
